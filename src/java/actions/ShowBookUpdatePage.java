 ...
 * @author ganest
 */
public class ShowBookUpdatePage implements Action {
   ...
   public boolean execute(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
      ...
      for(int i=0; i<editor.length; i++)
       ...
       book.setIsbn(isbn);
        try {
            ...
            Logger.getLogger(ShowBookUpdatePage.class.getName()).log(Level.SEVERE, null, ex);
        }       
      ...
      if(!bookId.equals("-1")){	
        book = DAOFactory.getDAOFactory(DAOFactory.EXIST).getBookDAO().updateBook(bookId, book ,collId);
      }else{
        book=DAOFactory.getDAOFactory(DAOFactory.EXIST).getBookDAO().insertBook(book ,collId);
      }
